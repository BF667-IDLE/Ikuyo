...
async function sendButtons(sock, jid, opts, sendOpts = {}) {
    ...
    if (header) {
        switch (header.type) {
            case 'image':
                msg.image = typeof header.media === 'string' ? { url: header.media } : header.media;
                msg.headerType = 2;
                break;
            case 'video':
                msg.video = typeof header.media === 'string' ? { url: header.media } : header.media;
                msg.headerType = 4;
                break;
            case 'document':
                msg.document = typeof header.media === 'string' ? { url: header.media } : header.media;
                msg.headerType = 3;
                break;
        }
    }
