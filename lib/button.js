...
// ============================================================
//  INTERNAL: Fix media for Baileys
// ============================================================

/**
 * Fix media object untuk Baileys.
 * Baileys memerlukan media dalam format Buffer untuk kestabilan.
...
async function fixMedia(media, type) {
    ...
    // Untuk image, convert ke Buffer untuk kestabilan
    if (type === 'image') {
        try {
...
async function sendButtons(sock, jid, opts, sendOpts = {}) {
    ...
    // Tambahkan header media jika ada
    if (header) {
        const fixedMedia = await fixMedia(header.media, header.type);
        switch (header.type) {
            case 'image':
                // Baileys butuh Buffer untuk image, bukan { url }
...
async function sendTemplateButton(sock, jid, opts, sendOpts = {}) {
    ...
    if (mediaType && media) {
        ...
        switch (mediaType) {
            case 'image':
                // Baileys butuh Buffer langsung, bukan { url }
                msg.image = fixedMedia;
                break;
            case 'video':
                msg.video = Buffer.isBuffer(fixedMedia) ? fixedMedia : { url: fixedMedia };
